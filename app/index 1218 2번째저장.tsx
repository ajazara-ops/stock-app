...
export default function App() {
  ...
  // 뒤로가기 핸들러 (Alert 처리 추가)
  useEffect(() => {
    const backAction = () => {
      // 1. 알림창(Alert)이 떠있다면 닫기 ('취소' 동작 수행)
      ...
      // 2. 주식 상세 모달이 열려있다면 닫기
      if (selectedStock) { setSelectedStock(null); return true; }
      
      // 3. 가이드 모달이 열려있다면 닫기
      if (showGuide) { setShowGuide(false); return true; }

      // 4. [수정] 히스토리 모드(리포트 상세)라면 히스토리 목록으로 복귀
      if (isHistoryMode) {
        setIsHistoryMode(false);
        setActiveView('HISTORY'); // 리포트 목록으로 이동
        setHistoryDate(null);
        return true;
      }
      
      // 5. 메인 화면(HOME)이 아니라면 메인으로 이동
      if (activeView !== 'HOME') {
        setActiveView('HOME');
        // 히스토리 모드였다면 해제하고 최신 데이터 로드 (혹시 모를 예외 처리)
        if (isHistoryMode) { 
            setIsHistoryMode(false); 
            setHistoryDate(null); 
      ...
      // 6. 그 외(메인 화면 등)에서는 기본 동작(앱 종료)
      return false;
    };

    const backHandler = BackHandler.addEventListener('hardwareBackPress', backAction);
    return () => backHandler.remove();
  }, [selectedStock, activeView, showGuide, isHistoryMode, alertConfig.visible]); // alertConfig.visible 의존성 추가
